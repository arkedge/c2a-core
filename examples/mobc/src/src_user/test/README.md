...
- 上記の Python 環境は [uv](https://docs.astral.sh/uv/) を用いてセットアップすること
...
```
uv sync --locked
uv run pytest -m real -v   # 実機の場合
uv run pytest -m sils -v   # SILSの場合
...
```
uv run pytest -m real -v ./src_user/applications/user_defined/test_tlm_mem_dump.py
```

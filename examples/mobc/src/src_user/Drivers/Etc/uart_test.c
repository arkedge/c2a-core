...
static FRM_ERR_CODE UART_TEST_load_driver_super_init_settings_(Framing* p_framing);
static FRM_ERR_CODE UART_TEST_analyze_rec_data_(FRM_StreamConfig* p_stream_config, void* p_driver);
...
FRM_INIT_ERR_CODE UART_TEST_init(UART_TEST_Driver* uart_test_driver, uint8_t ch, FRM_StreamRecBuffer* rx_buffers[FRM_STREAM_MAX])
{
  ...
  ret = FRM_init_streams(&(uart_test_driver->driver.super),
...
static FRM_ERR_CODE UART_TEST_load_driver_super_init_settings_(Framing* p_framing)
{
  ...
  // stream0の設定
  p_stream_config = &(p_framing->stream_config[UART_TEST_STREAM_FIX]);
  FRMSC_enable(p_stream_config);
  FRMSC_enable_strict_frame_search(p_stream_config);

  // 送信はする
  FRMSC_set_tx_frame(p_stream_config, UART_TEST_tx_frame_);  // 送る直前に中身をmemcpyする
  FRMSC_set_tx_frame_size(p_stream_config, 0);               // 送る直前に値をセットする

  // 定期的な受信はする
  FRMSC_set_rx_header(p_stream_config, UART_TEST_header_, UART_TEST_HEADER_SIZE);
  FRMSC_set_rx_footer(p_stream_config, UART_TEST_footer_, UART_TEST_FOOTER_SIZE);
  FRMSC_set_rx_frame_size(p_stream_config, 12);
  FRMSC_set_data_analyzer(p_stream_config, UART_TEST_analyze_rec_data_);

  // stream1の設定
  p_stream_config = &(p_framing->stream_config[UART_TEST_STREAM_VAR]);
  FRMSC_enable(p_stream_config);
  FRMSC_enable_strict_frame_search(p_stream_config);
  ...
  FRMSC_set_rx_header(p_stream_config, UART_TEST_header_, UART_TEST_HEADER_SIZE);
  FRMSC_set_rx_footer(p_stream_config, UART_TEST_footer_, UART_TEST_FOOTER_SIZE);
  FRMSC_set_rx_frame_size(p_stream_config, -1);    // 可変
  FRMSC_set_rx_framelength_pos(p_stream_config, UART_TEST_HEADER_SIZE);
  FRMSC_set_rx_framelength_type_size(p_stream_config, 2);
  FRMSC_set_rx_framelength_offset(p_stream_config, UART_TEST_HEADER_SIZE + UART_TEST_FOOTER_SIZE);
  FRMSC_set_data_analyzer(p_stream_config, UART_TEST_analyze_rec_data_);
...
FRM_REC_ERR_CODE UART_TEST_rec(UART_TEST_Driver* uart_test_driver)
{
  ...
  ret = FRM_receive(&(uart_test_driver->driver.super));
  ...
  if (FRMSC_get_rec_status(p_stream_config)->status_code == FRM_STREAM_REC_STATUS_FIXED_FRAME)
  {
    ret = FRM_analyze_rec_data(&(uart_test_driver->driver.super), UART_TEST_STREAM_FIX, uart_test_driver);
  }

  p_stream_config = &(uart_test_driver->driver.super.stream_config[UART_TEST_STREAM_VAR]);
  if (FRMSC_get_rec_status(p_stream_config)->status_code == FRM_STREAM_REC_STATUS_FIXED_FRAME)
  {
    ret = FRM_analyze_rec_data(&(uart_test_driver->driver.super), UART_TEST_STREAM_VAR, uart_test_driver);
  }
...
#ifndef SILS_FW
  Printf("DEC DATA: %d\n", FRMSC_get_rec_status(p_stream_config)->fixed_frame_len);
  for (i = 0; i < FRMSC_get_rec_status(p_stream_config)->fixed_frame_len; i++)
  {
    Printf("%02x ", FRMSC_get_rx_frame(p_stream_config)[i]);
  }
  Printf("             \n");
#else
  printf("DEC DATA: %d\n", FRMSC_get_rec_status(p_stream_config)->fixed_frame_len);
...
FRM_CMD_ERR_CODE UART_TEST_send(UART_TEST_Driver* uart_test_driver, uint8_t id)
{
  ...
  case 0:
    ...
    FRMSC_set_tx_frame_size(p_stream_config, 7);
    break;
  case 1:
    UART_TEST_tx_frame_[0] = 'i';
    UART_TEST_tx_frame_[1] = 'd';
  ...
  case 2:
    ...
    FRMSC_set_tx_frame_size(p_stream_config, 5);
    break;
  default:
    ...
    FRMSC_set_tx_frame_size(p_stream_config, 9);
    break;
  }

  ret = FRM_send_general_cmd(&(uart_test_driver->driver.super), UART_TEST_STREAM_FIX);

...
CDS_REC_ERR_CODE AOBC_rec(AOBC_Driver* aobc_driver)
{
  CDS_ERR_CODE ret;
  CDS_StreamConfig* p_stream_config;

  ret = CDS_receive(&(aobc_driver->driver.super));
  ...
  ret = CDS_analyze_rec_data(&(aobc_driver->driver.super), AOBC_STREAM_TLM_CMD, aobc_driver);
...
#if 0
// 非C2A系列はこのように書く
static CDS_ERR_CODE AOBC_analyze_rec_data_(CDS_StreamConfig* p_stream_config, void* p_driver)
{
  AOBC_Driver* aobc_driver = (AOBC_Driver*)p_driver;
  uint32_t     tlm_ver = CDS_ISSLFMT_get_tlm_version(p_stream_config);
  uint32_t     tlm_id  = CDS_ISSLFMT_get_tlm_id(tlm_ver, p_stream_config);
...
CDS_CMD_ERR_CODE AOBC_send_cmd(AOBC_Driver* aobc_driver, const CommonCmdPacket* packet)
{
  CDS_ERR_CODE ret;
  CDS_StreamConfig* p_stream_config;
  ...
  case AOBC_Cmd_CODE_TG_FORWARD_AS_ST_TLM:
    ret = CDS_send_req_tlm_cmd(&(aobc_driver->driver.super), AOBC_STREAM_TLM_CMD);
    break;
  default:
    ret = CDS_send_general_cmd(&(aobc_driver->driver.super), AOBC_STREAM_TLM_CMD);
    break;
  }

...
def test_c2a_comm_gsc():
    ...
    send_sub_obc_gsc_directly(USE_DUMMY_CMD_ID, ())
    check_gsc_ack(USE_DUMMY_CMD_ID, "SUC")

    send_sub_obc_gsc_via_mobc(USE_DUMMY_CMD_ID, ())
...
def test_c2a_comm_tlc():
    global g_sub_obc_rtc_cnt
    set_cmd_counter()

    wait_sec = 15
    wait_ti = 10 * wait_sec

    sub_obc_ti = get_sub_obc_ti()

    send_sub_obc_gsc_directly(c2a_enum.Cmd_CODE_TLCD_CLEAR_ALL_TIMELINE, (0,))
    send_sub_obc_tlc_directly(sub_obc_ti + wait_ti, USE_DUMMY_CMD_ID, ())
    check_tlc_ack(sub_obc_ti + wait_ti, USE_DUMMY_CMD_ID)
    send_sub_obc_gsc_directly(c2a_enum.Cmd_CODE_TLCD_CLEAR_ALL_TIMELINE, (0,))

    mobc_ti = get_mobc_ti()

    send_mobc_gsc(mobc_c2a_enum.Cmd_CODE_TLCD_CLEAR_ALL_TIMELINE, (0,))
    send_sub_obc_tlc_via_mobc(mobc_ti + wait_ti, USE_DUMMY_CMD_ID, ())
    check_mobc_tlc_ack(mobc_ti + wait_ti, USE_DUMMY_CMD_ID)

    # MOBC TL の発火の確認
    time.sleep(wait_sec)
    g_sub_obc_rtc_cnt += 1  # TL 経由で1つ実行
    check_rtc_ack(USE_DUMMY_CMD_ID, "SUC")
...
def test_c2a_comm_blc():
    ...
    send_sub_obc_gsc_directly(c2a_enum.Cmd_CODE_BCT_CLEAR_BLOCK, (USE_BCT_ID,))
    send_sub_obc_bct_directly(1, USE_DUMMY_CMD_ID, ())
    ckeck_bct_ack(1, USE_DUMMY_CMD_ID, USE_BCT_ID)

    send_sub_obc_gsc_directly(c2a_enum.Cmd_CODE_BCT_CLEAR_BLOCK, (USE_BCT_ID,))
    send_mobc_gsc(mobc_c2a_enum.Cmd_CODE_BCT_CLEAR_BLOCK, (USE_BCT_ID,))
    send_sub_obc_bct_via_mobc(1, USE_DUMMY_CMD_ID, ())
...
def test_c2a_comm_tlm_buffer():
    ...
    assert "CNT" == wings.util.send_rt_cmd_and_confirm(
        ope,
        mobc_c2a_enum.Cmd_CODE_TG_FORWARD_AS_RT_TLM,
        (TLM_APID_SUB_OBC, c2a_enum.Tlm_CODE_AOBC_AOBC),
        mobc_c2a_enum.Tlm_CODE_HK,
    )
    send_sub_obc_gsc_directly(c2a_enum.Cmd_CODE_TG_GENERATE_RT_TLM, (c2a_enum.Tlm_CODE_AOBC_AOBC,))
    time.sleep(2)
...
def set_cmd_counter():
    ...
    global g_sub_obc_gsc_cnt
    global g_sub_obc_rtc_cnt

    g_mobc_gsc_cnt = get_mobc_gsc_counter()
    g_sub_obc_gsc_cnt = get_sub_obc_gsc_counter()
    g_sub_obc_rtc_cnt = get_sub_obc_rtc_counter()
...
def send_sub_obc_gsc_directly(cmd_id, params):
    global g_sub_obc_gsc_cnt
    ope.send_rt_cmd(cmd_id, params, SUB_OBC, False)
    g_sub_obc_gsc_cnt += 1


def send_sub_obc_gsc_via_mobc(cmd_id, params):
    global g_mobc_gsc_cnt
    global g_sub_obc_rtc_cnt
    ope.send_rt_cmd(cmd_id, params, SUB_OBC, True)
    g_sub_obc_rtc_cnt += 1
    g_mobc_gsc_cnt += 1  # MOBC キュー を経由


def send_sub_obc_tlc_directly(ti, cmd_id, params):
    ope.send_tl_cmd(ti, cmd_id, params, SUB_OBC, False)


def send_sub_obc_tlc_via_mobc(ti, cmd_id, params):
    ope.send_tl_cmd(ti, cmd_id, params, SUB_OBC, True)


def send_sub_obc_bct_directly(ti, cmd_id, params):
    ope.send_bl_cmd(ti, cmd_id, params, SUB_OBC, False)


def send_sub_obc_bct_via_mobc(ti, cmd_id, params):
    ope.send_bl_cmd(ti, cmd_id, params, SUB_OBC, True)
...
def check_rtc_ack(exec_cmd, sts):
    ...
    assert tlm_SUB_HK[SUB_OBC + "_HK.OBC.GS_CMD.COUNTER"] == g_sub_obc_gsc_cnt
    assert tlm_SUB_HK[SUB_OBC + "_HK.OBC.RT_CMD.COUNTER"] == g_sub_obc_rtc_cnt
    assert tlm_SUB_HK[SUB_OBC + "_HK.OBC.RT_CMD.LAST_EXEC.ID"] == exec_cmd
    assert tlm_SUB_HK[SUB_OBC + "_HK.OBC.RT_CMD.LAST_EXEC.EXEC_STS"] == sts
...
def check_tlc_ack(ti, exec_cmd):
    ...
    assert tlm_SUB_HK[SUB_OBC + "_HK.OBC.GS_CMD.COUNTER"] == g_sub_obc_gsc_cnt
    assert tlm_SUB_HK[SUB_OBC + "_HK.OBC.RT_CMD.COUNTER"] == g_sub_obc_rtc_cnt
    assert tlm_SUB_HK[SUB_OBC + "_HK.OBC.TLC_GS.QUEUED"] == 1
    assert tlm_SUB_HK[SUB_OBC + "_HK.OBC.TLC_GS.NEXT_TIME"] == ti
    assert tlm_SUB_HK[SUB_OBC + "_HK.OBC.TLC_GS.NEXT_ID"] == exec_cmd
...
def check_mobc_tlc_ack(ti, exec_cmd):
    ...
    assert tlm_SUB_HK[SUB_OBC + "_HK.OBC.GS_CMD.COUNTER"] == g_sub_obc_gsc_cnt
    assert tlm_SUB_HK[SUB_OBC + "_HK.OBC.RT_CMD.COUNTER"] == g_sub_obc_rtc_cnt
    assert tlm_SUB_HK[SUB_OBC + "_HK.OBC.TLC_GS.QUEUED"] == 0
...
def ckeck_bct_ack(ti, exec_cmd, bct_id):
    ...
    assert tlm_SUB_HK[SUB_OBC + "_HK.OBC.GS_CMD.COUNTER"] == g_sub_obc_gsc_cnt
    assert tlm_SUB_HK[SUB_OBC + "_HK.OBC.RT_CMD.COUNTER"] == g_sub_obc_rtc_cnt
    assert tlm_SUB_HK[SUB_OBC + "_HK.BCT_BLK_PTR"] == bct_id
    assert tlm_SUB_HK[SUB_OBC + "_HK.BCT_CMD_PTR"] == 1
    assert tlm_SUB_HK[SUB_OBC + "_HK.BCT_REGD_TIME"] == ti
    assert tlm_SUB_HK[SUB_OBC + "_HK.BCT_REGD_ID"] == exec_cmd
...
def ckeck_mobc_bct_ack(ti, exec_cmd, bct_id):
    ...
    tlm_MOBC = wings.util.generate_and_receive_tlm(
        ope, mobc_c2a_enum.Cmd_CODE_TG_GENERATE_RT_TLM, mobc_c2a_enum.Tlm_CODE_MOBC
    )
    ...
    assert tlm_SUB_HK[SUB_OBC + "_HK.OBC.RT_CMD.COUNTER"] == g_sub_obc_rtc_cnt
    assert tlm_SUB_HK[SUB_OBC + "_HK.BCT_BLK_PTR"] == bct_id
    assert tlm_SUB_HK[SUB_OBC + "_HK.BCT_CMD_PTR"] == 0
...
def get_sub_obc_ti():
    return get_sub_obc_hk_tlm()[SUB_OBC + "_HK.SH.TI"]
...
def get_sub_obc_gsc_counter():
    return get_sub_obc_hk_tlm()[SUB_OBC + "_HK.OBC.GS_CMD.COUNTER"]


def get_sub_obc_rtc_counter():
    return get_sub_obc_hk_tlm()[SUB_OBC + "_HK.OBC.RT_CMD.COUNTER"]


def wait_for_sub_obc_hk_tlm():
    time.sleep(1.5)

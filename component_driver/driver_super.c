...
static uint8_t CDS_if_rx_buffer_[CDS_IF_RX_BUFFER_SIZE];    //!< IF_RX で受信するときの一次バッファ

/**
 * @brief  コマンド送信処理
 *
 *         CDS_send_general_cmd と CDS_send_req_tlm_cmdの共通部分
 ...
 * @retval CDS_ERR_CODE_ERR: IF_TX でのエラーあり
 ...
 * @param  p_super: ComponentDriverSuper 構造体へのポインタ
 * @param  stream:  どの config を使用するか．stream は 0-MAX なので，継承先で ENUM など宣言して使いやすくすればいいと思う．
 * @retval CDS_ERR_CODE_OK (0): 正常終了
 * @retval 0 以外: IF_TX の戻り値
 */
static int CDS_tx_(ComponentDriverSuper* p_super, uint8_t stream);
 ...
 * @return void
 */
static void CDS_analyze_rx_buffer_cueing_buffer_(CDS_StreamConfig* p_stream_config);
 ...
 *
 *         受信バッファのデータを走査し，必要なデータをフレームとして pickup する
 * @note   CDS_analyze_rx_buffer_variable_pickup_with_rx_frame_size_ との違いは，テレメ長データがフレームに含まれるか否か
 ...
 * @note   CDS_analyze_rx_buffer_variable_pickup_with_rx_frame_size_ から呼ばれることを想定
 * @param  p_stream_config: ComponentDriverSuper 構造体の CDS_StreamConfig
 * @return フレーム長
 */
...
/**
 * @brief  CDS_StreamConfig 構造体の初期化
 *
 *         CDS_StreamConfig 構造体を初期化し，デフォルト値で埋める．
 * @param  p_stream_config: ComponentDriverSuper 構造体の CDS_StreamConfig
 * @return CDS_ERR_CODE
 */
static CDS_ERR_CODE CDS_reset_stream_config_(CDS_StreamConfig* p_stream_config);

/**
 * @brief  CDS_StreamConfig 構造体のバリデーション
 * @param  p_super:         ComponentDriverSuper 構造体へのポインタ
 * @param  p_stream_config: ComponentDriverSuper 構造体の CDS_StreamConfig
 * @return CDS_ERR_CODE
 */
static CDS_ERR_CODE CDS_validate_stream_config_(const ComponentDriverSuper* p_super, CDS_StreamConfig* p_stream_config);
 ...
 * @param[in,out] stream_rec_buffer: クリアする CDS_StreamRecBuffer
 * @return void
 */
void CDS_clear_stream_rec_buffer_(CDS_StreamRecBuffer* stream_rec_buffer);

/**
 * @brief CDS_StreamRecBuffer の前方要素の削除
 * @note stream_rec_buffer は NULL でないことを仮定する
 * @param[in,out] stream_rec_buffer: 削除する CDS_StreamRecBuffer
 ...
 * @note stream_rec_buffer は NULL でないことを仮定する
 * @param[in,out] stream_rec_buffer: 追記する CDS_StreamRecBuffer
 ...
 * @note stream_rec_buffer は NULL でないことを仮定する
 * @param[in] stream_rec_buffer: CDS_StreamRecBuffer
 * @return 未処理バッファサイズ
 */
uint16_t CDS_get_unprocessed_size_from_stream_rec_buffer_(CDS_StreamRecBuffer* stream_rec_buffer);

/**
 * @brief CDS_StreamRecBuffer で指定サイズだけバッファを確定させる
 * @note stream_rec_buffer は NULL でないことを仮定する
 * @param[in,out] stream_rec_buffer: 確定させる CDS_StreamRecBuffer
 ...
 * @note stream_rec_buffer は NULL でないことを仮定する
 * @param[in,out] stream_rec_buffer: 前進させる CDS_StreamRecBuffer
                    ...
                    CDS_ERR_CODE (*load_init_setting)(ComponentDriverSuper* p_super))
{
  ...
  return CDS_init_streams(p_super, if_config, rx_buffers, load_init_setting);
}
                            ...
                            CDS_ERR_CODE (*load_init_setting)(ComponentDriverSuper* p_super))
{
  ...
  if (CDS_reset(p_super) != CDS_ERR_CODE_OK) return CDS_ERR_CODE_ERR;
  ...
  if (p_super->config.internal.load_init_setting(p_super) != CDS_ERR_CODE_OK) return CDS_ERR_CODE_ERR;
  ...
  if ( (*IF_init[p_super->interface])(p_super->if_config) != 0 ) return CDS_ERR_CODE_ERR;
...
CDS_ERR_CODE CDS_reset(ComponentDriverSuper* p_super)
{
  ...
  p_super->config.settings.rx_buffer_size_in_if_rx_          = CDS_IF_RX_BUFFER_SIZE;
...
CDS_ERR_CODE CDS_validate_config(ComponentDriverSuper* p_super)
{
  ...
  if (p_super->interface < 0 || p_super->interface >= IF_LIST_MAX) return CDS_ERR_CODE_ERR;
  if (p_super->if_config == NULL) return CDS_ERR_CODE_ERR;

  if (p_super->config.settings.rx_buffer_size_in_if_rx_ > CDS_IF_RX_BUFFER_SIZE) return CDS_ERR_CODE_ERR;
...
CDS_ERR_CODE CDS_clear_rx_buffer(ComponentDriverSuper* p_super)
{
  ...
  memset(CDS_if_rx_buffer_,
         0x00,
         sizeof(CDS_if_rx_buffer_));
...
CDS_ERR_CODE CDS_receive(ComponentDriverSuper* p_super)
{
  ...
  if (p_super->config.settings.should_monitor_for_rx_disruption_)
  {
    ...
    {
      p_super->config.info.rec_status_.rx_disruption_status = CDS_RX_DISRUPTION_STATUS_LOST;
    }
  }
  ...
  for (stream = 0; stream < CDS_STREAM_MAX; ++stream)
  {
    ...
    {
      p_stream_config->info.rec_status_.status_code = CDS_STREAM_REC_STATUS_DISABLE;
      continue;
    }
    ...
    {
      CDS_ERR_CODE ret = CDS_validate_stream_config_(p_super, p_stream_config);
      if (ret != CDS_ERR_CODE_OK)
      {
        p_stream_config->info.rec_status_.status_code = CDS_STREAM_REC_STATUS_VALIDATE_ERR;
        continue;
      }
    }
    ...
    {
      // RX失敗
      p_stream_config->info.rec_status_.status_code = CDS_STREAM_REC_STATUS_RX_ERR;
    ...
    else if (ret_rx == 0)
    {
      ...
      if (CDS_get_unprocessed_size_from_stream_rec_buffer_(p_stream_config->settings.rx_buffer_) > 0)
      {
    ...
    // 受信データ処理
    CDS_analyze_rx_buffer_(p_stream_config, rec_data_len);
  ...
  for (stream = 0; stream < CDS_STREAM_MAX; ++stream)
  {
    ...
    // テレメなどで見るときにノイズになるので，判定しないときは OK にしておく
    p_stream_config->info.rec_status_.tlm_disruption_status = CDS_STREAM_TLM_DISRUPTION_STATUS_OK;
    ...
    {
      p_stream_config->info.rec_status_.tlm_disruption_status = CDS_STREAM_TLM_DISRUPTION_STATUS_LOST;
    }
  }

  return (ret_rx < 0) ? CDS_ERR_CODE_ERR : CDS_ERR_CODE_OK;
}
...
CDS_ERR_CODE CDS_send_general_cmd(ComponentDriverSuper* p_super, uint8_t stream)
{
  ...
  if (!p_stream_config->settings.is_enabled_)
  {
    p_stream_config->info.send_status_.status_code = CDS_STREAM_SEND_STATUS_DISABLE;
    return CDS_ERR_CODE_OK;
  }
...
static CDS_ERR_CODE CDS_send_cmd_(ComponentDriverSuper* p_super, uint8_t stream)
{
  ...
  if (p_stream_config->internal.is_validation_needed_for_send_)
  {
    ...
    if (ret != CDS_ERR_CODE_OK)
    {
      p_stream_config->info.send_status_.status_code = CDS_STREAM_SEND_STATUS_VALIDATE_ERR;
      return CDS_ERR_CODE_ERR;
    }
  }

  p_stream_config->info.send_status_.ret_from_if_tx = CDS_tx_(p_super, stream);

  if (p_stream_config->info.send_status_.ret_from_if_tx != 0)
  {
    p_stream_config->info.send_status_.status_code = CDS_STREAM_SEND_STATUS_TX_ERR;
    return CDS_ERR_CODE_ERR;
  }

  p_stream_config->info.send_status_.status_code = CDS_STREAM_SEND_STATUS_OK;
  return CDS_ERR_CODE_OK;
}
...
static int CDS_tx_(ComponentDriverSuper* p_super, uint8_t stream)
{
  ...
  if (p_stream_config->settings.tx_frame_size_ == 0) return CDS_ERR_CODE_OK;
  if (p_stream_config->settings.tx_frame_ == NULL) return CDS_ERR_CODE_OK;
...
static int CDS_rx_(ComponentDriverSuper* p_super)
{
  ...
  {
    Printf("%02x ", CDS_if_rx_buffer_[i]);
    if (i % 4 == 3) Printf("   ");
  }
  Printf("\n");
#endif
                                  ...
                                  uint16_t rec_data_len)
{
  ...
  CDS_analyze_rx_buffer_pickup_(p_stream_config);

  // 次回のデータ受信に備えて，バッファの頭出し
  CDS_analyze_rx_buffer_cueing_buffer_(p_stream_config);
}
                                                 ...
                                                 uint16_t rec_data_len)
{
  ...
  if (buffer->is_frame_fixed)
  {
    ...
    CDS_drop_from_stream_rec_buffer_(buffer, buffer->pos_of_frame_head_candidate);   // 念のため

    if (p_stream_config->settings.is_strict_frame_search_)
    {
      // 厳格なフレーム探索なので，1 つの可能性も受信漏らさないように
      CDS_drop_from_stream_rec_buffer_(buffer, 1);
    }
    else
    {
      CDS_drop_from_stream_rec_buffer_(buffer, buffer->confirmed_frame_len);
    }
  ...
  ret = CDS_push_to_stream_rec_buffer_(buffer, CDS_if_rx_buffer_, rec_data_len);
  if (ret != CDS_ERR_CODE_OK)
  {
    // バッファがあふれるので，一旦古いバッファのクリア
    // TODO: この仕様は検討してもよいかも？（ユーザー選択にするとか）
    CDS_clear_stream_rec_buffer_(buffer);
    CDS_push_to_stream_rec_buffer_(buffer, CDS_if_rx_buffer_, rec_data_len);
...
static void CDS_analyze_rx_buffer_pickup_(CDS_StreamConfig* p_stream_config)
{
  ...
  if (p_stream_config->info.rec_status_.status_code == CDS_STREAM_REC_STATUS_FIXED_FRAME)
  {
    ...
    // FIXME: is_frame_fixed と CDS_STREAM_REC_STATUS_FIXED_FRAME あたいの混在がだるいので直す？
...
static void CDS_analyze_rx_buffer_cueing_buffer_(CDS_StreamConfig* p_stream_config)
{
  ...
  CDS_drop_from_stream_rec_buffer_(buffer, buffer->pos_of_frame_head_candidate);
}
...
static void CDS_analyze_rx_buffer_fixed_pickup_(CDS_StreamConfig* p_stream_config)
{
  ...
  else if (buffer->confirmed_frame_len < p->settings.rx_frame_size_ - p->settings.rx_footer_size_)
  {
    ...
    CDS_confirm_stream_rec_buffer_(buffer, pickup_data_len);     // pickup_data_len byte 確定
    p->info.rec_status_.status_code = CDS_STREAM_REC_STATUS_RECEIVING_DATA;

    // フッタがなく，data 受信仕切った場合はフレーム確定
    // これがないと，CDS_analyze_rx_buffer_fixed_ で
...
static void CDS_analyze_rx_buffer_variable_pickup_with_rx_frame_size_(CDS_StreamConfig* p_stream_config)
{
  ...
  CDS_StreamRecBuffer* buffer = p->settings.rx_buffer_;
  const uint16_t unprocessed_data_len = CDS_get_unprocessed_size_from_stream_rec_buffer_(buffer);
  uint32_t rx_frame_size = CDS_analyze_rx_buffer_get_framelength_(p_stream_config);      // まだ受信していない場合は不定値が入ることに留意すること！！
...
static void CDS_analyze_rx_buffer_variable_pickup_with_footer_(CDS_StreamConfig* p_stream_config)
{
  ...
  else
  {
    ...
    CDS_confirm_stream_rec_buffer_(buffer, processed_data_len);     // processed_data_len byte 確定
...
static void CDS_analyze_rx_buffer_finding_header_(CDS_StreamConfig* p_stream_config)
{
  ...
  CDS_confirm_stream_rec_buffer_(buffer, 1);     // ヘッダ 1 byte 目が見つかった
...
static void CDS_analyze_rx_buffer_receiving_header_(CDS_StreamConfig* p_stream_config)
{
  ...
  {
    CDS_confirm_stream_rec_buffer_(buffer, 1);     // ヘッダの次の 1 byte がみつかった
    p->info.rec_status_.status_code = CDS_STREAM_REC_STATUS_RECEIVING_HEADER;
    return;
  }
  else
  {
    // ヘッダが正しくなかった
    // CDS_STREAM_REC_STATUS_HEADER_MISMATCH になり，再びバッファを巻き戻してヘッダ探索を始める
    // その後 CDS_STREAM_REC_STATUS_FINDING_HEADER に戻る
                                                   ...
                                                   uint16_t rx_frame_size)
{
  ...
  // 期待されているフッタが受信できたか？
  // 受信できなかった場合， CDS_STREAM_REC_STATUS_FOOTER_MISMATCH になり，再びバッファを巻き戻してヘッダ探索を始める
  // その後 CDS_STREAM_REC_STATUS_FINDING_HEADER に戻る
...
static CDS_ERR_CODE CDS_reset_stream_config_(CDS_StreamConfig* p_stream_config)
{
  ...
  p->info.rec_status_.status_code                  = CDS_STREAM_REC_STATUS_DISABLE;
  p->info.rec_status_.fixed_frame_len              = 0;
  p->info.rec_status_.tlm_disruption_status        = CDS_STREAM_TLM_DISRUPTION_STATUS_OK;
  ...
  p->info.ret_from_data_analyzer_ = CDS_ERR_CODE_OK;
...
static CDS_ERR_CODE CDS_validate_stream_config_(const ComponentDriverSuper* p_super, CDS_StreamConfig* p_stream_config)
{
  ...
  if (!p->settings.is_enabled_) return CDS_ERR_CODE_OK;

  if (p->settings.tx_frame_size_  != 0 && p->settings.tx_frame_  == NULL) return CDS_ERR_CODE_ERR;
  if (p->settings.rx_header_size_ != 0 && p->settings.rx_header_ == NULL) return CDS_ERR_CODE_ERR;
  if (p->settings.rx_footer_size_ != 0 && p->settings.rx_footer_ == NULL) return CDS_ERR_CODE_ERR;

  if (p->settings.tx_frame_buffer_size_ >= 0)
  {
    if (p->settings.tx_frame_size_ > p->settings.tx_frame_buffer_size_) return CDS_ERR_CODE_ERR;
  }
  ...
  if (p->settings.rx_frame_size_ < 0)
  {
    ...
    if (p->settings.rx_framelength_pos_ < 0)
    {
      // フレームサイズデータがない場合
      // フッタの存在が必須
      if (p->settings.rx_footer_size_ == 0) return CDS_ERR_CODE_ERR;
    }
    else
    {
      if (p->settings.rx_header_size_ == 0) return CDS_ERR_CODE_ERR;       // 可変長かつヘッダなしは対応しない（固定長のようにして回避する．詳細はヘッダファイル参照）
      if (p->settings.rx_framelength_pos_ < p->settings.rx_header_size_) return CDS_ERR_CODE_ERR;    // フレームサイズがヘッダ（つまり固定値）に含まれることはありえないので
      if (!(p->settings.rx_framelength_type_size_ == 1 ||
            p->settings.rx_framelength_type_size_ == 2 ||
            p->settings.rx_framelength_type_size_ == 3 ||
            p->settings.rx_framelength_type_size_ == 4 )) return CDS_ERR_CODE_ERR;    // 現在は uint8 to uint32 のみ対応
      if (!(p->settings.rx_framelength_endian_ == ENDIAN_TYPE_BIG ||
            p->settings.rx_framelength_endian_ == ENDIAN_TYPE_LITTLE )) return CDS_ERR_CODE_ERR;
    }
  }
  ...
  {
    // ヘッダがあることが前提
    if (p->settings.rx_header_size_ == 0) return CDS_ERR_CODE_ERR;
  }

  if (p->settings.rx_buffer_ == NULL) return CDS_ERR_CODE_ERR;
  if (p->settings.rx_buffer_->buffer == NULL) return CDS_ERR_CODE_ERR;
  if (p->settings.rx_buffer_->capacity < p_super->config.settings.rx_buffer_size_in_if_rx_) return CDS_ERR_CODE_ERR;
  if (p->settings.rx_buffer_->capacity < p->settings.rx_frame_size_) return CDS_ERR_CODE_ERR;
  if (p->settings.rx_buffer_->capacity < p->settings.rx_header_size_ + p->settings.rx_footer_size_) return CDS_ERR_CODE_ERR;
...
CCP_CmdRet CDS_conv_cmd_err_to_ccp_cmd_ret(CDS_CMD_ERR_CODE code)
{
  switch (code)
  {
  case CDS_CMD_ILLEGAL_CONTEXT:
    return CCP_make_cmd_ret(CCP_EXEC_ILLEGAL_CONTEXT, (uint32_t)code);
  case CDS_CMD_ILLEGAL_PARAMETER:
    return CCP_make_cmd_ret(CCP_EXEC_ILLEGAL_PARAMETER, (uint32_t)code);
  case CDS_CMD_ILLEGAL_LENGTH:
    return CCP_make_cmd_ret(CCP_EXEC_ILLEGAL_LENGTH, (uint32_t)code);
  default:
    // ここに来るのは以下の 3 つ
    // CDS_CMD_OK
    // CDS_CMD_DRIVER_SUPER_ERR
    // CDS_CMD_UNKNOWN_ERR

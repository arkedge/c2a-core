...
typedef enum
{
  ...
  DS_CMD_DRIVER_SUPER_ERR   = 4,   //!< Framing 側，つまり配送の低レイヤーでエラーが起きた場合
...
typedef struct
{
  ...
  struct
  {
    DS_ERR_CODE (*load_init_setting)(Framing* p_super);   /*!< DS_init でロードする，ドライバの初期設定の設定関数
...
struct Framing
{
  ...
  DS_Config         config;                                 //!< Framing の設定

  DS_StreamConfig   stream_config[DS_STREAM_MAX];           /*!< Framing Stream
...
/**
 * @brief  確定したフレームを取得
 * @param  p_stream_config[in]: Framing 構造体の DS_StreamConfig

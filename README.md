...
- C2A の SILS runtime として，以下のような実装が存在する．
  - [c2a-dev-runtime](./dev-runtime)
    - 開発用の SILS runtime．これが導入済みの C2A user は基本的に `cargo run` すれば動作する．
    - ドキュメント: [docs/sils/c2a_dev_runtime.md](./docs/sils/c2a_dev_runtime.md)
...
- [`examples`](./examples) の C2A user は [c2a-dev-runtime](./dev-runtime) と [S2E User for C2A Core](https://github.com/ut-issl/s2e-user-for-c2a-core) での模擬に対応．
